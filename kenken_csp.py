...
def binary_ne_grid(kenken_grid):
    ...
    var_array 	 = [] #2-Dimension array
    for i in range(n):
        var_array.append(list())

    #add variable objects to csp object and var_array
    ...
    for i in range(n):#row number
        for j in range(n):#column number
            for k in range(len(var_array[i])):#idx in a row
                if k <= j:#note k starts from zero but k > j
                    continue
                var1 = var_array[i][j]
                var2 = var_array[i][k]
    ...
    for i in range(n):#row number
        for j in range(n):#column number
            for k in range(len(var_array[i])):#idx in a column
                if k <= i:#note k starts from zero but k > i
                    continue
                var1 = var_array[i][j]
                var2 = var_array[k][j]
...
def add(v_tuple, V):
    result = v_tuple[0]
    for i in range(1, len(v_tuple)):
        result += v_tuple[i]
    return result==V


def subtract(v_tuple, V):
    result = v_tuple[0]
    for i in range(1, len(v_tuple)):
        result -= v_tuple[i]
    return result==V
...
def divide(v_tuple, V):
    result = v_tuple[0]
    for i in range(1, len(v_tuple)):
		#result //= v_tuple[i]#integer division -> more strict constraint (more sup_tuple) and hence longer search time.
        result /= v_tuple[i]
    return result==V


def multiply(v_tuple, V):
    result = v_tuple[0]
    for i in range(1, len(v_tuple)):
        result *= v_tuple[i]
    return result==V
...
def kenken_csp_model(kenken_grid):
    ...
    for i in range(1, len(kenken_grid)):
        ...
        if len(kenken_grid[i]) == 2:
            ...
            i = int(str(cell)[0]) - 1#row number
            j = int(str(cell)[1]) - 1#col number
            ...
            for t in itertools.product(*all_domains):
                if opr   == ADD:
                    ...
                    for p in itertools.permutations(t):
                        if subtract(p, target_val):
                            sat_tuples.append(t)
                            break
                elif opr == DIVIDE:
                    for p in itertools.permutations(t):
                        if divide(p, target_val):
                            sat_tuples.append(t)
                            break
                elif opr == MULTIPLY:
                    if multiply(t, target_val):
                        sat_tuples.append(t)
                else:
                    pass
...
if __name__ == "__main__":
    ...
    #kenken_grid = [[3],[11, 2], [11,21,3,0],[12,22,2,1],[13,23,33,6,3],[31,32,5,0]]
    #kenken_grid = [[4],[11,21,6,3],[12,13,3,0],[14,24,3,1],[22,23,7,0],[31,32,2,2],[33,43,3,1],[34,44,6,3],[41,42,7,0]]
    #kenken_grid = [[4], [11, 21, 6, 3], [12, 13, 3, 0], [14, 24, 3, 1], [22, 23, 7, 0], [31, 32, 2, 2], [33, 43, 3, 1], [34, 44, 6, 3], [41, 42, 7, 0], [22, 4], [23, 3]]
    ...
    #kenken_grid = [[5], [12, 2], [11, 21, 4, 1], [12, 13, 2, 2], [14, 24, 1, 1], [15, 25, 1, 1], [22, 23, 9, 0], [31, 32, 3, 1], [33, 34, 44, 6, 3], [35, 45, 9, 0], [41, 51, 7, 0], [42, 43, 3, 1], [52, 53, 6, 3], [54, 55, 4, 1]]
    ...
    #kenken_grid = [[6],[12,2],[11,12,13,2,2],[14,15,3,1],[16,26,36,11,0],[21,22,23,2,2],[24,25,34,35,40,3],[31,41,51,61,14,0],[32,33,42,43,52,53,3600,3],[44,54,64,120,3],[45,46,55,56,1,1],[62,63,5,1],[65,66,5,0]]
