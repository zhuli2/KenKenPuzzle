...
class TestStringMethods(unittest.TestCase):
    ...
    def test_bne_grid_1(self):#test-case 1
        board = BOARDS[0]
        self.helper_bne_grid(board)

    @unittest.skipUnless(TEST_MODELS, "Not Testing Models.")
    def test_bne_grid_2(self):#test-case 2
        board = BOARDS[1]
        self.helper_bne_grid(board)
    
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")
    def test_props_1(self):#test-case 3
        board = BOARDS[0]
        self.helper_prop(board)
    
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")
    def test_props_2(self):#test-case 4
        board = BOARDS[1]
        self.helper_prop(board)
    
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")
    def test_props_3(self):#test-case 5
        board = BOARDS[2]
        self.helper_prop(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")
    def test_props_4(self):#test-case 6
        board = BOARDS[3]
        self.helper_prop(board, prop_GAC)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")   
    def test_props_5(self):#test-case 7
        board = BOARDS[4]
        self.helper_prop(board, prop_GAC)
    
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_MODELS, "Not Testing Propagators and Models.")
    def test_props_6(self):#test-case 8
        board = BOARDS[5]
        self.helper_prop(board, prop_GAC)
    
    @unittest.skipUnless(TEST_HEURISTICS, "Not Testing Heuristics.")
    def test_ord_mrv_1(self):#test-case 9
        a = Variable('A', [1])
        b = Variable('B', [1])
        c = Variable('C', [1])
        d = Variable('D', [1])
    ...
    @unittest.skipUnless(TEST_HEURISTICS, "Not Testing Heuristics.")
    def test_ord_mrv_2(self):#test-case 10
    ...
    def test_simple_FC(self):#test-case 11
        queens = nQueens(8)
    ...
    @unittest.skipUnless(TEST_PROPAGATORS, "Not Testing Propagotors.")
    def test_DWO_FC(self):#test-case 12
        queens = nQueens(6)
...
if __name__ == '__main__':
    print("test bt_search for nQueens CSP:")
    print("-"*40)   
    n = 6
    csp = nQueens(n)
    solver_nqueens = BT(csp)
    #solver_nqueens.bt_search(prop_FC, ord_mrv)
    #solver_nqueens.bt_search(prop_FC, ord_mrv, val_lcv)
    #solver_nqueens.bt_search(prop_FC, ord_dh)
    #solver_nqueens.bt_search(prop_FC, ord_dh, val_lcv)

    #solver_nqueens.bt_search(prop_GAC, ord_mrv)
    #solver_nqueens.bt_search(prop_GAC, ord_mrv, val_lcv)
    #solver_nqueens.bt_search(prop_GAC, ord_dh)
    solver_nqueens.bt_search(prop_GAC, ord_dh, val_lcv)      
    print("-"*40)    
    
    print("testing kenken_csp_model:")    
    unittest.main()
